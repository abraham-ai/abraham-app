...
const BLESS_PRICE_WEI = "20000000000000"; // 2e13 wei
...
export async function POST(req: Request) {
  try {
    ...
    const { sessionId, messageId, cid } = body as {
      sessionId?: string;
      messageId?: string;
      cid?: string;
    };
    if (!sessionId || !messageId || !cid)
      return NextResponse.json(
        { error: "sessionId, messageId, cid required" },
        { status: 400 }
      );
    ...
    const allowed = isAllowedTierA(policy, {
      ...
      valueWei: BigInt(BLESS_PRICE_WEI),
    });
    if (!allowed)
